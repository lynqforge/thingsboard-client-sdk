...
class Provision_Callback : public Callback<void, JsonDocument const &> {
  public:
    ...
    /// @param isGateway The cloud will treat this as a gateway
    Provision_Callback(Access_Token, function callback, char const * provision_device_key, char const * provision_device_secret, char const * device_name = nullptr, uint64_t const & timeout_microseconds = 0U, Callback_Watchdog::function timeout_callback = nullptr, boolean isGateway = false);
    ...
    /// @param isGateway The cloud will treat this as a gateway
    Provision_Callback(Device_Access_Token, function callback, char const * provision_device_key, char const * provision_device_secret, char const * access_token, char const * device_name = nullptr, uint64_t const & timeout_microseconds = 0U, Callback_Watchdog::function timeout_callback = nullptr, boolean isGateway = false);
    ...
    /// or if the connection could not be established, default = nullptr
    /// @param isGateway The cloud will treat this as a gateway
    Provision_Callback(Basic_MQTT_Credentials, function callback, char const * provision_device_key, char const * provision_device_secret, char const * username, char const * password, char const * client_id, char const * device_name = nullptr, uint64_t const & timeout_microseconds = 0U, Callback_Watchdog::function timeout_callback = nullptr, boolean isGateway = false);
    ...
    /// or if the connection could not be established, default = nullptr
    /// @param isGateway The cloud will treat this as a gateway
    Provision_Callback(X509_Certificate, function callback, char const * provision_device_key, char const * provision_device_secret, char const * hash, char const * device_name = nullptr, uint64_t const & timeout_microseconds = 0U, Callback_Watchdog::function timeout_callback = nullptr, boolean isGateway = false);
    ...
    /// @brief Gets the boolean stating whether this is a gateway or not
    /// by the Provision_Callback and therefore decides what response we will receive from the server
    /// @return String containing the used credentials type
    boolean Get_Gateway() const;
  ...
  private:
    ...
    bool              m_gateway = {};              // is this a gateway ?
